  ...
  searchResult(options = {}) {
    ...
    if (this._searchConfig.searchMode === 'local') {
      ...
      if(querys.subQuerys) {
        ...
        query = AntiSearchSource._addInSearchQuery(query, subQuery);
      }
...
this.AntiSearchSource = {
  ...
  _SearchConfig: Match.Where(function(config) {
    check(config, {
      collection: String,
      fields: [String],
  ...
  _buildSearchQuery: function(configEntry) {
    ...
    if (configEntry.searchString) {
      ...
      // searchStringQueries = configEntry.fields.map(function(fieldName) {
      //   let $orEntry = {};
      //   $orEntry[fieldName] = {
  ...
  _addInSearchQuery: function(query, subQuery) {
  ...
  create: function(config) {
    return new AntiSearchSourceClient(config);
  }
};
